...
(function() {
  var Promise, config, credentials, crypto, flags, fs, httpRequest, klarna, parseResponse, request, wrapper;
  ...
  parseResponse = function(error, response, body) {
    if (error) {
      return {
        type: 'HTTP Request',
        code: error.code,
      ...
      return new Promise(function(resolve, reject) {
        return request.post(httpRequest.options(data, id), function(error, response, body) {
          if ((response.statusCode != null) && response.statusCode === 200) {
            ...
            return reject(parseResponse(error, response, body));
          }
        });
      });
    };
